...
class MIFBeat : public Message 
{
    ...
    //vector<__u32> traget_export_load_list;
...
public:
  ...
  void encode_payload(uint64_t features) override {
    for (vector<migration_decision_t>::iterator it = my_decision.begin();it!=my_decision.end();it++){
      decision_map[(*it).target_import_mds]=(*it).traget_export_load;
    }
  ...
  void decode_payload() override {
    ...
    //::decode(traget_export_load_list, payload);
    /*my_decision.clear();
    for (unsigned int i = 0;i<target_import_mds_list.size();i++){
      migration_decision_t temp_decision = {target_import_mds_list[i],traget_export_load_list[i]};
      my_decision.push_back(temp_decision);
    }*/
  }
};
