...
void Server::respond_to_request(MDRequestRef& mdr, int r)
{
  ...
  dout(0) << ADSLTAG_MIGRATION_CORRE_REQUEST << " " << now2str() << " Finishing request: " << ceph_mds_op_name(req->get_op()) << " on " << req->get_path() << " and " << req->get_path2() << " retried " << mdr->retry << " times." << dendl;
#endif
...
#ifdef ADSLTAG_MIGRATION_CORRE_REQUEST
  dout(0) << ADSLTAG_MIGRATION_CORRE_REQUEST << " " << now2str() << " Handling request: " << ceph_mds_op_name(req->get_op()) << " on " << req->get_path() << " and " << req->get_path2() << " retried " << mdr->retry << " times." << dendl;
